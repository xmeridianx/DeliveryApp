...
import com.example.meatdeliveryapp.databinding.FragmentSweetsBinding
...
class SweetsFragment : Fragment(), OnProductClickListener {
    private lateinit var binding: FragmentSweetsBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentSweetsBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapters = Array(5) { AdapterProduct(mutableListOf(), this) }
        productRefs = arrayOf(
            categoryRef.child("Sweets").child("Chocolate"),
            categoryRef.child("Sweets").child("Chocolate_bar") ,
            categoryRef.child("Sweets").child("Cookies"),
            categoryRef.child("Sweets").child("Gum"),
            categoryRef.child("Sweets").child("Jam"))

        thread = Array(5) { Thread() }
        ...
        thread[0] = Thread {
            ...
            handler.post {
                binding.recycler1.adapter = adapters[0]
    ...
    thread[3] = Thread {
        val productList4 = loadData(productRefs[3])

        adapters[3] = AdapterProduct(productList4, this)
        val layoutManager = LinearLayoutManager(
            requireContext(),
            LinearLayoutManager.HORIZONTAL, false
        )
        handler.post {
            binding.recycler4.adapter = adapters[3]
            binding.recycler4.layoutManager = layoutManager
    ...
    thread[4] = Thread {
        val productList5 = loadData(productRefs[4])

        adapters[4] = AdapterProduct(productList5, this)
        val layoutManager = LinearLayoutManager(
            requireContext(),
            LinearLayoutManager.HORIZONTAL, false
        )
        handler.post {
            binding.recycler5.adapter = adapters[4]
            binding.recycler5.layoutManager = layoutManager
